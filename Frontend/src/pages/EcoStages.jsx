  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-surface-800 tracking-tight">ECO Stages</h1>
          <p className="text-sm text-surface-500 mt-1">Configure the sequential workflow stages for engineering changes</p>
        </div>
      ...
      <div className="max-w-3xl">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-surface-100 rounded-xl border border-surface-200 shadow-sm p-2">
          {ECO_STAGES.length === 0 ? (
            <div className="text-center p-12 text-surface-500">No stages configured.</div>
          ) : (
              ...
              {ECO_STAGES.map((stage, idx) => (
                <div key={idx} className="flex items-center gap-4 p-4 bg-white border border-surface-200 rounded-lg group hover:border-primary-300 transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-surface-400 mt-0.5">Workflow step sequence #{idx + 1}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <button className="text-surface-400 hover:text-primary-600 transition-colors">Edit</button>
                    {idx > 0 && idx < ECO_STAGES.length - 1 && (
                      <button className="text-surface-400 hover:text-danger-600 transition-colors">Remove</button>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="mt-4 p-4 rounded-lg bg-surface-50 border text-surface-600 text-sm flex items-start gap-3">
          ...
          <p>ECOs progress sequentially downwards. Drag and drop to reorder stages. The first stage is always the Draft state, and the final state is Closed.</p>
        </div>
      </div>
    </div>
  );
