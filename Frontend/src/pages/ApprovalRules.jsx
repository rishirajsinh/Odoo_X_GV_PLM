  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-surface-800 tracking-tight">Approval Rules</h1>
          <p className="text-sm text-surface-500 mt-1">Manage conditional logic for ECO routing and required signatures</p>
        </div>
          ...
          approvalRules.map(rule => (
            <div key={rule.id} className="bg-white rounded-xl border border-surface-200 p-5 shadow-sm hover:border-primary-300 transition-colors relative flex flex-col h-full">
              ...
              <div className="space-y-3 flex-1">
                ...
                <div>
                  <p className="text-xs text-surface-500 mb-1">Required Role</p>
                  <p className="text-sm font-medium text-surface-700">{rule.requiredRole}</p>
                </div>
                <div>
                  <p className="text-xs text-surface-500 mb-1">Applies at Stage</p>
              ...
              <div className="mt-4 pt-4 border-t border-surface-100 flex gap-2">
                <button className="flex-1 py-1.5 text-sm font-medium text-surface-600 bg-surface-50 hover:bg-surface-100 rounded-lg transition-colors">Edit</button>
                <button className="flex-1 py-1.5 text-sm font-medium text-danger-600 bg-danger-50 hover:bg-danger-100 rounded-lg transition-colors">{rule.status === 'Active' ? 'Disable' : 'Enable'}</button>
              </div>
            </div>
          ))
        )}
